...
from UI.Tkinter.Tkinter_UI import DataSetViewer
...
class UIPrediction:
    ...
    def create_ui(self):
        ...
        Label(loadmodel_panel, text="Chọn model:").grid(row=0, column=0)

        self.selected_model = StringVar()
        self.selected_model.set("")  # mặc định rỗng
        model_list = self.get_model_files()
        default_value = model_list[0] if model_list else ""
        self.selected_model = StringVar()
        self.selected_model.set(default_value)
        self.model_dropdown = OptionMenu(loadmodel_panel, self.selected_model, default_value, *model_list)
    ...
    def do_save_model(self):
        # Hỏi người dùng có muốn lưu model hay không
        answer = messagebox.askyesno("Confirm Save", "Bạn có chắc chắn muốn lưu mô hình mới không?")
        if not answer:
            messagebox.showinfo("Info", "Hủy lưu mô hình.")
            return
        ...
        filename = f"../../Data/housingmodel_{now}.zip"

        result = FileUtil.savemodel(self.lm, filename)
        if result:
            messagebox.showinfo("Info", f"Đã lưu mô hình: {filename}")
            # Sau khi lưu xong thì cập nhật danh sách trong OptionMenu
            self.update_model_list()
        else:
            messagebox.showinfo("Error", "Lưu mô hình thất bại!")
        pass

    def do_load_model(self):
        selected_model = self.selected_model.get()
        if not selected_model:
            messagebox.showwarning("Warning", "Vui lòng chọn mô hình để tải!")
            return

        model_path = os.path.join("../../Data", selected_model)
        if not os.path.exists(model_path):
            messagebox.showerror("Error", f"Không tìm thấy file: {model_path}")
            return

        self.lm = FileUtil.loadmodel(model_path)
        messagebox.showinfo("Info", f"Đã tải thành công mô hình: {selected_model}")
        pass
    ...
    def get_model_files(self):
        """Lấy danh sách tất cả model .zip trong thư mục Data"""
        os.makedirs("../../Data", exist_ok=True)
        files = [f for f in os.listdir("../../Data") if f.endswith(".zip")]
        return files

    def update_model_list(self):
        """Cập nhật OptionMenu danh sách model"""
        menu = self.model_dropdown["menu"]
        menu.delete(0, "end")
        for model in self.get_model_files():
