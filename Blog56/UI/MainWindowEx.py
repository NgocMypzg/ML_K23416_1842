...
class MainWindowEx(Ui_MainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.checkBoxGender.stateChanged.connect(self.update_columns)
        self.checkBoxAge.stateChanged.connect(self.update_columns)
        self.checkBoxPaymentMethods.stateChanged.connect(self.update_columns)
        ...
        methods = ["Cash", "Credit", "Debit card"]
        self.comboBoxPaymentMethods.addItems(methods)
        self.comboBoxPaymentMethods.currentTextChanged.connect(self.on_payment_method_changed)
    ...
    def update_columns(self):
        ...
        if self.checkBoxPaymentMethods.isChecked():
            selected.append("payment_method")

        self.columns_input = selected
    ...
    def processPrediction(self):
        if self.radioButtonFemale.isChecked():
            gender = self.radioButtonFemale.text()
        elif self.radioButtonMale.isChecked():
            gender = self.radioButtonMale.text()
        else:
            gender = None
        age = int(self.lineEditAge.text())

        if len(self.purchaseLinearRegression.trainedmodel.columns_input)==3:
            predicted_price = self.purchaseLinearRegression.predictPriceFromGenderAndAgeAndPayment(gender, age, self.payment_method)
        else:
